...
#define D
/*
 * do { char		b         [PATH_MAX]; procPath(b); fprintf(stderr,
 * "%s:%d %s\n", b, getpid(), __FUNCTION__); fflush(stderr); } while (0)
 */
#define DD  // do { char b[PATH_MAX]; procPath(b); fprintf(stderr, "%s:%d
///%s \ n ", b, getpid(), __FUNCTION__); fflush(stderr); } while (0)
...
__xstat(int v, const char *restrict path, struct stat *restrict buf) {
    int r;
    static int (*o__xstat)(int, const char *restrict, struct stat *restrict) =
        0;
    D;
    R(__xstat);
...
__xlstat(int v, const char *restrict path, struct stat *restrict buf) {
    int r;
    static int (*o__xlstat)(int, const char *restrict, struct stat *restrict) =
        0;
    D;
    R(__xlstat);
...
__fxstatat(int v, int fd, const char *path, struct stat *buf, int flag) {
    ...
    D;
    if (fd != AT_FDCWD) {
        static int (*o__fxstatat)(int, int, const char *, struct stat *restrict,
                                  int) = 0;
        R(__fxstatat);
